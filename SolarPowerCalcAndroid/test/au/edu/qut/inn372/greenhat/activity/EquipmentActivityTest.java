...
public class EquipmentActivityTest extends
		ActivityInstrumentationTestCase2<EquipmentActivity> {
	...
	protected void setUp() throws Exception {
		...
		parentActivity.runOnUiThread(new Runnable() {
			public void run() {
				parentActivity.switchTab(TabbedActivity.EQUIPMENT_ID);
			}
		});
		activity = (EquipmentActivity) activityMonitor.waitForActivityWithTimeout(TIMEOUT);
	}
	
	@Override
	protected void tearDown() throws Exception {
		if(activity.getState() == EquipmentActivity.STATE_PAUSED){
			...
			while(activity.getState() == EquipmentActivity.STATE_PAUSED) {};
		}
		activity.finish();
	...
	public void testSaveData() {
		...
		//The following loop waits until the state has paused before proceeding (since it is in a separate thread)
		while(activity.getState() == EquipmentActivity.STATE_NORMAL) {};
	...
	public void testNextActivity() {
		...
		final Button button = (Button) activity.findViewById(R.id.buttonEquipment_Next);
		activity.runOnUiThread(new Runnable() {
			public void run() {
				button.performClick();
			}
	...
	public void testBackActivity() {
		...
		final Button button = (Button) activity.findViewById(R.id.buttonEquipment_Back);
		activity.runOnUiThread(new Runnable() {
			public void run() {
				button.performClick();
			}
